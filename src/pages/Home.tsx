  ...
  return (
    <>
      <section className="relative h-svh overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center justify-end pb-24 text-center px-6">
          ...
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.2 }}>
            <Link to={localePath('/catalog')} className="mt-8 inline-block border border-paper/40 px-10 py-4 uppercase tracking-[0.3em] text-xs hover:bg-paper hover:text-ink transition-colors">
              {t('home.cta')}
            </Link>
          </m.div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-24">
        <Reveal>
          <h2 className="font-display text-2xl sm:text-3xl uppercase tracking-widest mb-10">{t('home.newCollection')}</h2>
        </Reveal>
      ...
      <section className="py-12">
        <div className="flex gap-4 overflow-x-auto px-4 sm:px-6 snap-x">
          {campaigns.map(src => (
            <img
              key={src} src={src} alt={t('home.campaignAlt')} loading="lazy"
              className="h-[60vh] w-auto object-cover snap-start shrink-0"
            />
          ))}
        </div>
        <Reveal className="text-center mt-10">
          <Link to={localePath('/lookbook')} className="uppercase tracking-[0.3em] text-xs border-b border-mist pb-1 hover:text-mist transition-colors">
            {t('home.viewLookbook')}
          </Link>
        </Reveal>
      </section>
