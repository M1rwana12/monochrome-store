  ...
  return (
    <header className="fixed top-0 inset-x-0 z-40 mix-blend-difference text-paper">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 h-16 flex items-center justify-between">
        <Link to="/" className="font-display font-bold tracking-[0.3em] text-base sm:text-lg">
          MONOCHROME
        </Link>
        <nav className="flex items-center gap-4 sm:gap-6 text-xs sm:text-sm uppercase tracking-widest">
          {nav.map(n => (
            <NavLink
              key={n.to}
              to={n.to}
